...
**2026-07-06 (late pm) — Icon-library unification complete. Both Next apps now use a single icon
...
frontend). Both `dev` branches pushed and in sync with `origin` (admin `de87b4b`, frontend `c74b82c`).
See ledger D5.**
...
- `alt-admin` — `dev` @ `de87b4b` (**icon migration P2**, pushed). Icon P1 (iconify→lucide) at `d2565d9`;
  before that `main`=`ed2e679` (PR #1 merge), forgot-password repoint `70e646c`, tooling pass `29548e3`,
  admin-invite **UI** `d3ed5db` / `f43543f`.
- `alt-frontend` — `dev` @ `c74b82c` (**icon migration P2**, pushed). Icon P1 (iconify→lucide) at
  `3033a18`; before that `main`=`f6b61e8` (PR #1 merge), tooling pass `41ae698` + form-shape `default-`
  prefix rename + a `react-select` → Headless UI Listbox select refactor.
...
- **Merge `dev` → `main`** on admin + frontend when ready — the icon migration (P1+P2) currently lives
  only on `dev`; `main` is still at the PR #1 merge.
