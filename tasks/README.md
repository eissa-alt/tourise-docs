...
| 036 | [category-reply-to-mailboxes](036-category-reply-to-mailboxes/TASK.md) | done (code) — a guest's **reply** now reaches the team that owns their audience. Task 015 added the per-flow override mechanism; this is the mailbox plan on top of it. `CategorySmtpConfigsSeeder` builds one account per Reply-To from the client's Phase 1 sheet — From stays `noreply@tourise.com` as `TOURISE 2027` on every account, only Reply-To varies. **The first cut derived it as `{category-slug}@tourise.com` and only 1 of 10 addresses was real**, so it is now an explicit map; several categories share one inbox (4 dignitary → `protocol@`, Speaker + Moderator → `programming@`). Also: SMTP bulk edit (select-then-edit, badges-style), Reply-To column on the SMTP listing, category **View more**, and the email template named on the invitation-collections listing. Seeded on production 2026-09-13 — 7 accounts, **9 categories wired, 0 left alone**. ⚠️ Provider not yet verified to send as `tourise.com`, the 7 mailboxes must exist, and General Registration has no mailbox (the client's sheet gives it `contact@`, which the owner excluded) |
