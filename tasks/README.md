...
| 033 | [fork-port-back](033-fork-port-back/TASK.md) | **in-progress** — port back what the three clones (`122-gfeai-v2`, `123-pif-pep-v2`, `124-ewc-2026-v2`) fixed. Three-pass audit done and committed (docs `7f049d3` + `212d58a`); **nothing ported yet.** Working list = [FORK_PORT_BACK_FINDINGS.md](../upgrades/FORK_PORT_BACK_FINDINGS.md): ~60 verified items / 87 checkboxes, 17 of them one-liners. ⚠️ Binding process: **one item at a time, no commit until the owner reviews the diff.** Top risks: multi-use invitation links die after the first registration; the tokenized complete-data link creates a duplicate guest in 2 of 3 shapes; `GET /admin/guest-data-offline` 500s on every call. Also records baseline defects no fork fixes (2 public forced-500 test hooks, unthrottled public `/pdf/{id}`, cross-client storage buckets on live PDF paths) |
| 034 | [tourise-form-shapes](034-tourise-form-shapes/TASK.md) | **in-progress** — two new form shapes on the existing form-shapes pattern: **Tourise Private** (fields 10–13 gated on Industry = Media, Marketing & Influence) and **Tourise Public**. Requiredness stays 100% admin-driven via `categories.mandatory_fields` — modelled on `one-step/step-1.tsx`, **not** on `fours-steps/step-1.tsx`, which hardcodes `isRequired`. New tourism industry dataset (15 values) kept separate from `industry-types-select` so stored values aren't orphaned. Additive migration for `social_media_channel_1/2`. ⚠️ **Visa Assistance reuses `valid_visa` inverted** (owner decision): answering Yes stores `valid_visa = false`, which is exactly what makes a guest e-visa eligible. Gates green (524 backend tests); **not yet committed, not browser-tested** |

| 035 | [bundled-mandatory-fields](035-bundled-mandatory-fields/TASK.md) | done (code) — the per-shape mandatory-field catalogue moves **back into the admin bundle** (`data/mandatory-fields-by-form-shape.tsx` behind `getMandatoryFieldsByFormShape`), reversing Task 033's `form_shape_fields` delivery and matching `128-pif-psf-2026` Task 039. Seeded data meant a stale picker per environment: backend `6c7ae93` added the `default-media` list to the seeder only, so **Media Registration rendered "No results found"** until a re-seed. Parity proved against `FormShapeFieldsSeeder::catalogue()` before deleting anything — all 7 shapes identical, order included (111 rows). Model, controller, seeder, 2 routes and tests deleted; `2026_09_06_000001` drops the table. Also closes a flagged exposure — the routes sat in the plain `auth:sanctum` group with no `admin.can` gate. Gates green (pint + phpstan + 540 tests, 547 → 540; admin type-check/build/check:rbac). Backend `eaa6a1d`, admin `b08a930`; **migrate + push + browser QA pending** |

| 036 | [system-email-templates](036-system-email-templates/TASK.md) | **done (code)** — the five system emails (login codes, admin invite, the two security alerts) stop being hardcoded blades and become `type=system` rows in `email_templates`, opened in the Waypoint builder like any other template and seeded with the invitation's own header + black footer. `SystemEmail::render()` + `resolveSystem()` supply the per-send values (the code, an IP, a user agent) that no guest carries; all five senders switched and **14 blade views deleted**. The base view stops splicing a second footer in before `</body>` — that duplicate is what started the task. Admin gets a System templates tab, the builder in a modal with language/template switchers behind an unsaved-changes guard, and a variable palette scoped by template type. Four bugs fixed en route: Arabic rendered LTR in sent mail, bare domains were auto-linked in Gmail blue (ported from `113-pif-directors-gathering`), cloning a system template 500'd on its unique `system_key`, and deactivating one would have silently stopped every admin login code. Ledger D48 + D49. Backend `c92ab56`, admin `1f0ee8e`; **not pushed**; prod migrate + seed + one re-save of the live invitation template pending |

| 037 | [per-category-reply-to](037-per-category-reply-to/TASK.md) | **done (code)** — guests reply to a category-specific address (`media@`, `vip@`) while every mail still leaves the one authenticated `no-reply@` mailbox. Two nullable columns on `smtp_configs` + one `Config::set` in `DynamicSmtpService`; **no category column and no new category UI** — the per-category selection already exists via `categories.smtp_config_id`, so the workflow is clone the account, change its Reply-To, point the category at it. Reaches all ~15 send paths at once because `MailManager` reads `mail.reply_to` on the same line as `mail.from`, so no Mailable or template changed. ⚠️ The key is written on every apply, **null included** — a conditional write would leak one category's Reply-To onto the next queued job. Ledger D50. Gates green (pint + 644 tests, 641 → 644; admin type-check/build/check:rbac). Backend `ce437a7`, admin `69d734b` — committed after owner review, **not pushed**; then dev migrate, admin configuration and browser QA |
