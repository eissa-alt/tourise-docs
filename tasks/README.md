...
| 021 | [seating](021-seating/TASK.md) | done (code) — Seating Plan Manager (Vite/React SPA from 120) ported into the baseline as a **4th sub-app** `alt-static-basecode-seating` (API-wire, no UI merge); backend schema/endpoints + dedicated `seating` RBAC feature (view/check_in/manage) + `Guest` attendance bridge, SPA retargeted, admin deep-link launch button; ledger D42; gates green (backend 480 tests, admin type-check/check:rbac, seating build); **on feature branches, NOT pushed** — pending owner merge/push + seating remotes + `.env` + migrate + live QA |
