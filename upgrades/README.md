...
| [FORK_PORT_BACK_FINDINGS.md](FORK_PORT_BACK_FINDINGS.md) | findings + working checklist | 🚧 in progress (opened 2026-08-06, section 4 closed + section 5 started 2026-08-08) — what to copy back from the three clones (`122-gfeai-v2`, `123-pif-pep-v2`, `124-ewc-2026-v2`). ~60 verified items, 89 checkboxes; **25 done, 64 left**. Also records defects in this baseline that **no fork fixes**. ⚠️ Its per-item *summaries* have been wrong five times so far (file/line refs always right) — **open the file before editing**. |
| [FORK_PORT_BACK_LEFTOVERS.md](FORK_PORT_BACK_LEFTOVERS.md) | parked work + open decisions | 🅿️ open (2026-08-08) — everything hit while doing the port-back and deliberately **not** acted on, with the reason: 12 remaining `env()`-under-`config:cache` calls, 8 dead filter keys, the `check_in_time` ↔ `checked_in_at` duplication, and the list of what still needs verifying in a browser. |
